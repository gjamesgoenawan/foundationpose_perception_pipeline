...
def export_component(wrapper, inputs, path, input_names, output_names, verify=False, skip_existing=False):
    ...
    for name, reference, value in zip(output_names, expected, actual, strict=True):
        np.testing.assert_allclose(value, reference.detach().cpu().numpy(), rtol=1e-3, atol=1e-3,
...
def export_sam3_models(
    ...
    with patch.object(builder, "_create_vit_backbone", partial(builder._create_vit_backbone, use_rope_real=True)), \
        ...
        image = torch.rand(1, 3, 1008, 1008, device=device) * 2 - 1
        tokens = model.backbone.language_backbone.tokenizer(["metal part"], context_length=32).to(device)
